...
var getPattern = (label, next) => {
  ...
  if (match) {
    ...
    if (!patternCache[cacheKey3]) {
      if (match[2]) {
        patternCache[cacheKey3] = next && next[0] !== ":" && next[0] !== "*" ? [cacheKey3, match[1], new RegExp(`^${match[2]}(?=/${next})`)] : [label, match[1], new RegExp(`^${match[2]}$`)];
      } else {
        patternCache[cacheKey3] = [label, match[1], true];
      }
    }
...
// src/utils.ts
var ALLOWED_FORMATS = ["ttml", "yrc", "lrc", "eslrc"];
var DEFAULT_FALLBACK_ORDER = ["yrc", "lrc", "eslrc"];
var isValidFormat = (format) => {
  if (!format) return false;
  return ALLOWED_FORMATS.includes(format);
};
var buildRawUrl = (id, format) => {
...
var buildExternalApiUrl = (id, externalApiBaseUrl) => {
  if (!externalApiBaseUrl) {
    throw new Error("External API base URL is not configured.");
  }
  return `${externalApiBaseUrl}?id=${encodeURIComponent(id)}`;
};
var LYRIC_LINE_REGEX = /^\[(?:\d{2}:\d{2}\.\d{2,3}|\d+,\d+)\]/;
var filterLyricLines = (rawLyrics) => {
  if (!rawLyrics) {
    return null;
  }
...
// src/lyricService.ts
var EXTERNAL_API_BASE_URL = process.env.EXTERNAL_NCM_API_URL;
async function fetchRepoLyric(id, format, logger) {
...
async function fetchExternalLyric(id, specificFormat, logger) {
...
async function handleFixedVersionSearch(id, fixedVersionQuery, logger) {
...
async function findTtmlInRepo(id, logger) {
...
async function findInRepoFallbacks(id, fallbackQuery, logger) {
  let fallbackOrder;
  if (fallbackQuery) {
    fallbackOrder = fallbackQuery.split(",").map((f) => f.trim().toLowerCase()).filter((f) => isValidFormat(f) && f !== "ttml");
...
async function findInExternalApi(id, logger) {
  logger.info(`LyricService: Trying external API fallback.`);
  const externalResult = await fetchExternalLyric(id, void 0, logger);
  if (externalResult.status === "found") {
    return { found: true, id, format: externalResult.format, source: "external", content: externalResult.content };
  }
  if (externalResult.status === "error") {
...
async function searchLyrics(id, options) {
  ...
  if (repoFallbackResult) {
    return repoFallbackResult;
  }
  return findInExternalApi(id, logger);
}

// node_modules/.pnpm/@hono+node-server@1.14.1_hono@4.7.8/node_modules/@hono/node-server/dist/vercel.mjs
...
// node_modules/.pnpm/@hono+node-server@1.14.1_hono@4.7.8/node_modules/@hono/node-server/dist/index.mjs
...
var toRequestError2 = (e) => {
  if (e instanceof RequestError2) {
    return e;
  }
  return new RequestError2(e.message, { cause: e });
};
var GlobalRequest2 = global.Request;
var Request3 = class extends GlobalRequest2 {
  constructor(input, options) {
...
var newRequestFromIncoming2 = (method, url, incoming, abortController) => {
  const headerRecord = [];
  const rawHeaders = incoming.rawHeaders;
  for (let i = 0; i < rawHeaders.length; i += 2) {
    const { [i]: key, [i + 1]: value } = rawHeaders;
  ...
  if (method === "TRACE") {
    init.method = "GET";
    const req = new Request3(url, init);
    Object.defineProperty(req, "method", {
      get() {
        return "TRACE";
      }
  ...
  return new Request3(url, init);
};
var getRequestCache2 = Symbol("getRequestCache");
var requestCache2 = Symbol("requestCache");
var incomingKey2 = Symbol("incomingKey");
var urlKey2 = Symbol("urlKey");
var abortControllerKey2 = Symbol("abortControllerKey");
var getAbortController2 = Symbol("getAbortController");
...
var newRequest2 = (incoming, defaultHostname) => {
  ...
  const host = (incoming instanceof Http2ServerRequest2 ? incoming.authority : incoming.headers.host) || defaultHostname;
  if (!host) {
...
var buildOutgoingHttpHeaders2 = (headers) => {
  const res = {};
  if (!(headers instanceof Headers)) {
    headers = new Headers(headers ?? void 0);
  }
...
var responseCache2 = Symbol("responseCache");
var getResponseCache2 = Symbol("getResponseCache");
var cacheKey2 = Symbol("cache");
var GlobalResponse2 = global.Response;
var Response22 = class _Response2 {
  #body;
  #init;
  [getResponseCache2]() {
  ...
  constructor(body, init) {
    ...
    if (typeof body === "string" || typeof body?.getReader !== "undefined") {
      let headers = init?.headers || { "content-type": "text/plain; charset=UTF-8" };
      if (headers instanceof Headers) {
        headers = buildOutgoingHttpHeaders2(headers);
      }
      ;
      this[cacheKey2] = [init?.status || 200, body, headers];
    }
  }
};
[
...
var stateKey2 = Reflect.ownKeys(new GlobalResponse2()).find(
  (k) => typeof k === "symbol" && k.toString() === "Symbol(state)"
);
if (!stateKey2) {
  console.warn("Failed to find Response internal state key");
}
function getInternalBody2(response) {
  if (!stateKey2) {
    return;
  }
  if (response instanceof Response22) {
    response = response[getResponseCache2]();
  }
  const state = response[stateKey2];
  return state && state.body || void 0;
}
var X_ALREADY_SENT2 = "x-hono-already-sent";
var webFetch2 = global.fetch;
if (typeof global.crypto === "undefined") {
...
global.fetch = (info, init) => {
  ...
  return webFetch2(info, init);
};
var regBuffer2 = /^no$/i;
var regContentType2 = /^(application\/json\b|text\/(?!event-stream\b))/i;
var handleRequestError2 = () => new Response(null, {
  status: 400
});
var handleFetchError2 = (e) => new Response(null, {
  status: e instanceof Error && (e.name === "TimeoutError" || e.constructor.name === "TimeoutError") ? 504 : 500
});
var handleResponseError2 = (e, outgoing) => {
  const err = e instanceof Error ? e : new Error("unknown error", { cause: e });
  if (err.code === "ERR_STREAM_PREMATURE_CLOSE") {
    console.info("The user aborted a request.");
  } else {
...
var responseViaCache2 = (res, outgoing) => {
  const [status, body, header] = res[cacheKey2];
  if (typeof body === "string") {
    header["Content-Length"] = Buffer.byteLength(body);
    outgoing.writeHead(status, header);
    outgoing.end(body);
    ...
    outgoing.writeHead(status, header);
    return writeFromReadableStream2(body, outgoing)?.catch(
      (e) => handleResponseError2(e, outgoing)
    );
  }
};
var responseViaResponseObject2 = async (res, outgoing, options = {}) => {
  if (res instanceof Promise) {
    if (options.errorHandler) {
      try {
        res = await res;
  ...
  const internalBody = getInternalBody2(res);
  if (internalBody) {
    const { length, source, stream } = internalBody;
    if (source instanceof Uint8Array && source.byteLength !== length) {
    } else {
      ...
      if (typeof source === "string" || source instanceof Uint8Array) {
        ...
        await writeFromReadableStream2(stream, outgoing);
      }
      return;
    }
  }
  if (res.body) {
    ...
    if (transferEncoding || contentEncoding || contentLength || // nginx buffering variant
    accelBuffering && regBuffer2.test(accelBuffering) || !regContentType2.test(contentType)) {
      outgoing.writeHead(res.status, resHeaderRecord);
      await writeFromReadableStream2(res.body, outgoing);
    } else {
      const buffer = await res.arrayBuffer();
      resHeaderRecord["content-length"] = buffer.byteLength;
      outgoing.writeHead(res.status, resHeaderRecord);
...
var getRequestListener2 = (fetchCallback, options = {}) => {
...
if (process.env.NODE_ENV !== "production" && !process.env.VERCEL) {
  console.log(`Using external API URL: ${EXTERNAL_API_BASE_URL2}`);
